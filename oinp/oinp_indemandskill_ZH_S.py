...
# 简体中文：OINP 雇主担保 - 紧缺职业类别打分项
scoring_rules_in_demand = {
    "工作职位：NOC 职业大类": {
        "职业大类 0 / 2 / 3": 10,
        "职业大类 7": 7,
        "职业大类 1 / 9": 5,
...
def run_in_demand_gui():
    ...
    for category, options in scoring_rules_in_demand.items():
    ...
    def calculate_score():
        ...
        for cat, var in variables.items():
            choice = var.get()
            if choice in scoring_rules_in_demand[cat]:
                total += scoring_rules_in_demand[cat][choice]
